...
const TransactionList: React.FC<TransactionListProps> = ({ transactions, className }) => {
  return (
    <div className={cn('glass rounded-xl p-4 bg-white/10', className)}>
        ...
        {transactions.map((transaction, index) => (
          ...
          >
            ...
            <div className="text-right">
              ...
              <div className="text-xs text-white/80">{transaction.time}</div>
            </div>
          </motion.div>
        ))}
      </div>
...
const TransactionIcon: React.FC<{ type: Transaction['type'] }> = ({ type }) => {
  switch (type) {
    case 'receive':
      return (
        <div className="w-8 h-8 rounded-full bg-green-500/30 flex items-center justify-center text-green-400">
          <ArrowDownRight size={16} />
        </div>
      );
    case 'send':
      return (
        <div className="w-8 h-8 rounded-full bg-red-500/30 flex items-center justify-center text-red-400">
          <ArrowUpRight size={16} />
        </div>
      );
    case 'pending':
      return (
        <div className="w-8 h-8 rounded-full bg-yellow-500/30 flex items-center justify-center text-yellow-400">
          <Clock size={16} />
        </div>
      );
  }
